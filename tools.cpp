...
int takePicture(int number, double ms)
{
  //Open/Create the bg file
  std::ofstream file("images/bg.txt",std::ios::out);
  if(!file)
    return -1;
  //VideoCapture cap(1);
  VideoCapture cap(CV_CAP_ANY);
  ...
  for(int i=0;i<number;i++)
  {
    ...
    if (!img.data)
	continue;
    imwrite("images/im"+to_string(i)+".jpg",img); // Save current pict
    file << "images/im"+to_string(i)+".jpg" << std::endl; // Add the pict to the bg.txt
  }
  file.close();
}
